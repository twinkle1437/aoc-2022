const fs = require('fs');
const data = fs.readFileSync('./input/day14.txt', 'utf8');
...
const init = () => {
	const line = (from, to) => {
		...
		for (let i = min[0]; i <= max[0]; i++)
			for (let j = min[1]; j <= max[1]; j++) screen[j][i] = 1;
	};

	let screen = Array.from({ length: maxY + 3 }, () => Array(maxX).fill(0));
	caves.forEach((point) =>
		point.forEach((p, i) => i && line(point[i - 1], p))
	);
	return screen;
};

const dropSand = (screen, part2) => {
	let sand = sandStart.slice(),
		thru = false;
	...
	while (true) {
		...
		if (screen[sand[1] + 1][sand[0]] == 0) {
			sand[1]++;
			continue;
		}
		if (screen[sand[1] + 1][sand[0] - 1] == 0) {
			sand[1]++;
			sand[0]--;
			continue;
		}
